...
# 1. 页面基础设置
st.set_page_config(
    page_title="Passion Meme Fixed",
    page_icon="✨",
    layout="wide",
    initial_sidebar_state="collapsed"
)

# 2. 隐藏 Streamlit 原生 UI
st.markdown("""
    <style>
...
<body>
    ...
    <script>
        ...
        // 稳定的 Bliss 壁纸链接 (Web Archive)
        const blissUrl = "https://web.archive.org/web/20230206142820if_/https://upload.wikimedia.org/wikipedia/en/d/d2/Bliss_%28Windows_XP%29.png";

        // === 真正的多色彩虹渐变库 (Based on your reference) ===
        const rainbowGradients = [
            "linear-gradient(180deg, #FF0000, #FF7F00, #FFFF00, #00FF00, #0000FF, #4B0082, #9400D3)", // 经典全彩虹
            "linear-gradient(45deg, #ff9a9e, #fad0c4, #fad0c4, #a18cd1, #fbc2eb)", // 梦幻糖果
            "linear-gradient(135deg, #667eea, #764ba2, #6B8DD6, #8E37D7)", // 赛博紫蓝
            "linear-gradient(to right, #4facfe, #00f2fe, #43e97b, #38f9d7)", // 极光青绿
            "linear-gradient(to bottom, #fa709a, #fee140, #ff9a9e, #fecfef)", // 落日暖色
            "linear-gradient(120deg, #d4fc79 0%, #96e6a1 100%)", // 清新绿
            "linear-gradient(to right, #eea2a2 0%, #bbc1bf 19%, #57c6e1 42%, #b49fda 79%, #7ac5d8 100%)", // 故障艺术
            "linear-gradient(90deg, #00C9FF 0%, #92FE9D 100%)", // 亮青
            "linear-gradient(to top, #fbc2eb 0%, #a6c1ee 100%)", // 蓝粉
            "linear-gradient(to right, #ffecd2 0%, #fcb69f 100%)", // 蜜桃
            "linear-gradient(to right, #ff8177 0%, #ff867a 0%, #ff8c7f 21%, #f99185 52%, #cf556c 78%, #b12a5b 100%)" // 强烈红粉
        ];

        function randomColor() { return `hsl(${Math.floor(Math.random() * 360)}, 100%, 50%)`; }

        // === 修正后的分词逻辑 (Intl.Segmenter) ===
        function segmentText(text) {
            text = text.trim();
            if (!text) return [];
            
            // 1. 如果包含空格，视为英文句子或手动分词 -> 按空格拆
            if (text.includes(' ')) {
                return text.split(/\s+/).filter(w => w.length > 0);
            }

            // 2. 如果没有空格，使用智能分词
            // 现代浏览器(Chrome 87+)都支持 Intl.Segmenter
            if (window.Intl && Intl.Segmenter) {
                try {
                    ...
                    // 降级方案
                    console.warn("Segmenter failed, falling back");
                    return text.split('');
                }
            } else {
                // 不支持的旧浏览器，降级为按字拆分
        ...
        function setBg(type) {
            if (type === 'white') canvas.style.background = '#ffffff';
                ...
                // 使用外链图片
                canvas.style.background = `url('${blissUrl}') center/cover no-repeat`;
            }
        }
